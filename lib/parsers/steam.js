  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(url, {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        ...
        if (!loc.includes('wishlistdata') || redirectsLeft <= 0)
          return reject(new Error(`Steam redirected away from wishlist data (→ ${loc}). Check that the profile URL is correct and the wishlist is public.`));
        return get(loc, redirectsLeft - 1).then(resolve).catch(reject);
      }
      let raw = '';
      res.on('data', c => raw += c);
      res.on('end', () => {
        ...
        if (raw.trimStart().startsWith('<'))
          return reject(new Error('Steam returned HTML instead of JSON — wishlist may be private or the URL is incorrect'));
